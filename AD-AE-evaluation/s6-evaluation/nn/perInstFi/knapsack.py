...
def getItemsUsed(w,c):
	...
	while (i >= 0 and currentW >=0):
		...
		if (i==0 and c[i][currentW] >0 )or (c[i][currentW] != c[i-1][currentW] and i != 0):
			marked[i] =1
...
if (len(sys.argv)!=3):
	...
	print("Example:")
	print("python3 knapsack.py 1-2,2-5,3-10 12")
	quit()
...
#knapsack_output_file = open("Result/knapsack_result.txt", "a")
#knapsack_output_file.write("max-weight: %d, max-profit: %d\n" % (maxCost, answer[0]))
#knapsack_output_file.write("optimal indices:")
print("max-weight: %i, max-profit: %.2f" % (maxCost, answer[0]))
print("optimal indices:")
for i in range(len(answer[1])):
	if (answer[1][i] != 0):
		#print i+1,
                #knapsack_output_file.write(" %i"%i)
		print(" %i"%i)
