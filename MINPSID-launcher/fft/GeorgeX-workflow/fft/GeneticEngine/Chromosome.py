...
class Chromosome:
    def __init__(self, bounds):
        ...
        emu = [4,6,8,10,12,14]
        minDiff = 100
        if int(self.x[0]) not in emu:
                for e in emu:
                        if abs(int(self.x[0]) - e) < minDiff:
                                minDiff = abs(int(self.x[0]) - e)
                self.x[0] = int(self.x[0]) - minDiff
			
        self.inputString = "-m" + str(self.x[0]) + " -p1 -n" + str(self.x[1]) + " -l" + str(self.x[2]) + " -t"
        ################################
