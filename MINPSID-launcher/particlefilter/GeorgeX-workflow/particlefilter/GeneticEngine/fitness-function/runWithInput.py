...
def generate_weighted_list(inp_string):
    # Generate Static CFG
    genStatCFGCMD = "opt -load ../../../LLVM-3.4-Passes/GenerateStaticCFG.so ./particlefilter-llfi_index.ll -GenStatCFG > temp_out"
    BB_start_idx = subprocess.getoutput(genStatCFGCMD).split('\n')
    os.system("rm -rf temp_out")
    # Generate Weighted Dynamic CFG
    preCMD1 = "opt -load ../../../LLVM-3.4-Passes/RecordControlFlow.so ./particlefilter-llfi_index.ll -RecordCF -o unlinkable.ll"
    ...
    preCMD5 = "clang++ readable.ll -o readable"
    os.system(preCMD1 + " && " + preCMD2 + " && " + preCMD3 + " && " + preCMD4 + " && " + preCMD5)
    os.system("./readable " + inp_string)
    os.system("rm -rf unlinkable.ll external.ll linkable.ll readable.ll readable")
