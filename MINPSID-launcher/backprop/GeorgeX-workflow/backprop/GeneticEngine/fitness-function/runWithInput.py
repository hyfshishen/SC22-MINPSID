...
def generate_weighted_list(inp_string):
    # Generate Static CFG
    genStatCFGCMD = "opt -load ../../../LLVM-3.4-Passes/GenerateStaticCFG.so ./backprop-llfi_index.ll -GenStatCFG > temp_out"
    BB_start_idx = subprocess.getoutput(genStatCFGCMD).split('\n')
    os.system("rm -rf temp_out")
    # Generate Weighted Dynamic CFG
    preCMD1 = "opt -load ../../../LLVM-3.4-Passes/RecordControlFlow.so ./backprop-llfi_index.ll -RecordCF -o unlinkable.ll"
